...
protected:
  void SetUp() override {
    ...
    Stack_ptr = std::make_unique<Stack>();
...
TEST_P(StackTest, Try1) {
  ...
  // файла.
  bool Stack_empty = Stack_ptr->empty();
  bool StandardStack_empty = StandardStack_ptr->is_empty();
  ASSERT_EQ(Stack_empty, StandardStack_empty);
  char command;
  while (file >> command) {
    if (command == 'p') {
      ...
      // изменился.
      Stack_empty = Stack_ptr->empty();
      StandardStack_empty = StandardStack_ptr->is_empty();
      ASSERT_EQ(Stack_empty, StandardStack_empty);
      if (!Stack_empty) {
      ...
      size_t Stack_value = Stack_ptr->top();
      size_t Stack_max = Stack_ptr->max();
